...
\todoin{ToDo: More smartcard-based works, FPGAs.}
...
%\begin{itemize}
\paragraph{Practical MPC computations:} \cite{BJSV15} MPC secure statistical analysis of income tax for Estonian government. \cite{BKKRST16} statistically analyzed database with 600k students and 10M tax records. \cite{KW15} suggested to use MPC to implement satellite collision prevention system securely evaluating collision detection algorithm using sensitive location and trajectory using secret sharing and floating point operations.
    
\paragraph{True2F~\cite{DCMBR18}} is a simple application of two-party computation to protect from untrusted U2F tokens. Motivated by ROCA \cite{2017-ccs-nemec} in which autors shown the well-known token generated easily-factorable RSA moduli which deteriorated system security. 
In True2F they designed lightweight 2PC protocols for collaborative signature key generation to ensure it has proper entropy contributed by both browser and token. They propose firewalled ECDSA signatures, jointly produced by browser and token in such a way that a) secrets remain private on token and b) token is blocked from exfiltrating secret signing key in the signature randomness.
The proposed modifications can run on the same hardware, server-side requires no modifications.
% Verifiable identities.
    
\paragraph{OPAQUE~\cite{JKX18}} shown how to compile asymmetric password-authenticated key exchange protocols (aPAKE) to OPAQUE protocols, protecting from offline pre-computation attacks as aPAKE protocols usually send hashing \emph{salt} to the host, the login key is derived as $k=H(pass, salt)$. The attacker can collect the salt in a targeted attack, pre-compute $H()$ tables using large word lists and then after server breach immediately recover user password using the tables. OPAQUE uses UC-secure MPC protocol in a way client does not learn the salt and server does not learn the password. The protocol is very effective and provides perfect forward secrecy, supports client-side hardened derivations, e.g., with scrypt and thresholding scheme so attacker has to attack on $t$ servers simultaneously to mount the attack.
    
%awesome-mpc
\paragraph{Compilers:} Fairplay~\cite{MNPS04} was the first published compiler for 2PC based on garbled circuits. Compilers and general MPC systems undergo massive active development since then. The \cite{sok19} gives an extensible up-to-date review of the numerous practically usable MPC systems. Tools list is depicted in table \ref{tbl:compilers}. 
...
\newpage
\begin{itemize}
    \item Real-world multiparty examples, SoK, compilers. OPAQUE

    \item UC secure framework, what can and cannot be done, setup assumptions, tokenzzzz
    
    \item TOKEN WORLD, assumptions, results... stateless, statefull, bounds, token complexities. use Katz and others
    
    \item Real-world MPC with tokens, True2F, 
\end{itemize}

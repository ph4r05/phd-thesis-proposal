...
\thesissetup{
    ...
    abstract      = {
    	...
    	The computation model consists of multiple heterogeneous nodes. A node is either a powerful generic device such as a cloud computation server or a very resource-restricted device, e.g., with low computational power, memory, and energy, such as a smartcard.
    	We aim to study MPC in the context of smartcards, to distribute the necessary trust among multiple parties to remove the single point of failure.
    	The primary focus is on practical applications increasing the security level in various usage scenarios like digital signatures or collaborative secret generation. The research will try to answer a question on how to practically realize MPC on smardcards and what are the current limits, as general MPC methods are quite computationally intensive and thus infeasible to run on smartcards.
    			},
...
This dissertation thesis proposal aims to introduce motivation for the study of the selected fields, cover state of the art, present my research plan and results achieved so far.
...
\begin{figure}[h]
	...
	\caption{Architecture of the \emph{Myst}, scalable MPC system designed in \cite{2017-ccs-mavroudis}. IC stands for integrated circuits, which are instantiated by smartcards in our work. From \cite{2017-ccs-mavroudis}.}
	\label{fig:myst_arch}
\end{figure}

...
Yao's work has been extended by \cite{GMW87} to a multi-party case and with malicious adaptive nodes using only one-way functions. They make use of Verifiable Secret Sharing (VSS) and agreement protocols. The work is based on boolean circuit but compared to \cite{Yao86} each party evaluates the circuit independently with shares of other players inputs. This work is a completeness theorem for distributed protocols with honest majority. Two protocols were proposed, the first for semi-honest setting which is secure with $m-1$ corrupted nodes, the other in the malicious settings secure with honest majority (correctness and privacy). Bounds are tight in this model assuming absolute security. 
...
plan model, ZK simulation inspired, \url{http://web.cs.ucla.edu/~rafail/PUBLIC/57.pdf}

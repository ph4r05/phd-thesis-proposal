...
On the other hand, we will analyze whether pre-programmed smart cards acting like a black box are properly implementing security-related functionalities. E.g., if Rabin Miller basis is fixed a pseudoprime could be generated which passes the test without actually being a prime. Such an attack breaks the system as we can bypass hardness assumptions of traditional ECC. Consequently, we will try to design protocols for collaborative parameter generation and attestations based on zero knowledge (ZK) proofs to avoid mentioned problems.
